...
// ULBounds retrieves the bbox in Degrees
func ULBounds(tile TileID) Bbox {
	leftTop := Ul(tile)
	nextTile := TileID{tile.X + 1, tile.Y + 1, tile.Z}
	rightBottom := Ul(nextTile)
	return Bbox{leftTop.Lng, rightBottom.Lat, rightBottom.Lng, leftTop.Lat}
